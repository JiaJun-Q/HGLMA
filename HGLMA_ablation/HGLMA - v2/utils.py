...
def process_data():
    ...
    # Load incidence matrices
    df_pos = pd.read_csv('incidence_matrix_pos.csv')
    hg_pos = create_hypergraph(df_pos, metabolite_count)
    hg_neg = create_hypergraph(pd.read_csv('incidence_matrix_neg.csv'), metabolite_count)

    print(hg_pos)

    # Generate Node2Vec embeddings based on clique expansion
    # Use 128 dimensions as standard for Node2Vec
    metabolite_features = get_node2vec_features(df_pos, metabolite_count, embedding_dim=128)

    return g, hg_pos, hg_neg, metabolite_features, reaction_count, metabolite_count


def get_node2vec_features(incidence_df, num_nodes, embedding_dim=128):
    # Construct Clique Expanded Graph
    G = nx.Graph()
    G.add_nodes_from(range(num_nodes))

    # Clique expansion: For each hyperedge, connect all constituent nodes
    grouped = incidence_df.groupby('hyperedge_id')['node_id'].apply(list)

    for nodes in tqdm(grouped, desc="Constructing Clique Graph"):
        if len(nodes) > 1:
    ...
    # Run Node2Vec
    node2vec = Node2Vec(G, dimensions=emb_dim, walk_length=walk_len, num_walks=10, workers=4, quiet=True)
    model = node2vec.fit(window=10, min_count=1, batch_words=4)

    # Extract embeddings
    embeddings = []
    for i in range(num_nodes):
        # Ensure we get the vector for each node index
        if str(i) in model.wv:
            embeddings.append(torch.tensor(model.wv[str(i)]))
        else:
