...
class Model(nn.Module):
    def __init__(self, in_channels: int,
        ...
        super().__init__()
        # Input dim is now 128 (Node2Vec default)
        self.mapping_layer = feature_mapping_mlp(128, in_channels)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, diag_mask, mask=None):
        ...
        dynamic = dynamic.permute(1, 2, 0, 3).contiguous().view(sz_b, len_q, -1)
        static = v.view(n_head, sz_b, len_q, d_v)
        static = static.permute(1, 2, 0, 3).contiguous().view(sz_b, len_q, -1)
