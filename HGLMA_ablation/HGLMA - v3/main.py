...
if __name__ == "__main__":
    ...
    for xml_file in xml_files:
        ...
        for fold, (train_set, valid_set) in enumerate(kf.split(np.arange(reaction_count))):
            ...
            net = Model(args.in_dim, args.hid_dim, args.out_dim, args.num_dgnn, args.num_hgnn, use_bn=True)
            ...
            for epoch in tqdm(range(args.external_epochs), desc=f"Training Fold {fold}"):
                net, classifier_model, fold_best_metrics = train(args, net, classifier_model, m_emb, g, hg_pos, hg_neg, train_set, valid_set, metabolite_count, reaction_count, loss_f,
                      ...
                      optimizer=[optimizer], epochs=args.internal_epochs, batch_size=batch_size, fold=fold, model_name=model_name)
